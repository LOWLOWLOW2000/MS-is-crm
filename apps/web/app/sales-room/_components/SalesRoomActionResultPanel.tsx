...
const matchActionValueFromResultLabel = (actionResult: string): string => {
  const hit = RESULT_OPTIONS.find((o) => o.label === actionResult || o.value === actionResult)
  if (hit) return hit.value
  if (actionResult.includes('留守電')) return '留守電'
  return ''
}
...
export function SalesRoomActionResultPanel({ companyName = '（現在の企業）', pageLink = '/sales-room' }: SalesRoomActionResultPanelProps) {
  ...
  const beginEditTopThreadItem = useCallback(() => {
    ...
    if (top.formSnapshot) {
      ...
      setAction(matchActionValueFromResultLabel(top.actionResult))
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-1">
                        <div className="min-w-0 flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-xs text-gray-500">
                        ...
                        {showEdit && (
                          <button
                            ...
                            className="shrink-0 rounded border border-blue-400 bg-white px-2 py-0.5 text-xs font-medium text-blue-700 hover:bg-blue-50"
                          >
                            編集
                          </button>
                        )}

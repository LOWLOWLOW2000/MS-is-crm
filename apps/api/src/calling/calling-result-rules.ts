...
/** 正規名 11 種（calling-record と整合） */
export const ALL_CALLING_RESULTS = [...CALLING_RESULT_VALUES] as const;

/** 「不在」以外の結果フラグ（同一扱いの母集団に含める用） */
export const CALLING_RESULTS_EXCEPT_ABSENT = ALL_CALLING_RESULTS.filter((r) => r !== '不在');

/** 未着電（旧留守電・不通は正規化で未着電に寄せる）の件数 */
export const countVoicemailAndUnreachable = (records: { result: string }[]): number =>
  records.reduce((n, r) => {
    const x = normalizeCallingResult(r.result);
    return n + (x === '未着電' ? 1 : 0);
  }, 0);

/** 未着電がこの回数以上なら「相手が受電した」相当として扱う */
 ...
 * - 再架電・担当NG は1件ごとに接続扱い（旧名も正規化して判定）
 * - 上記が無く未着電系が閾値以上のグループは接続1件として加算
 */
